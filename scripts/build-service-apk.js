...
console.log('🚀 [1/5] Preparando archivos web para PediGochos Servicios...');
execSync('node scripts/build-service-app.js', { stdio: 'inherit', cwd: rootDir });

console.log('⚙️ [2/5] Configurando Capacitor para PediGochos Servicios...');
fs.copyFileSync(
...
<resources>
    <string name="app_name">PediGochos Servicios</string>
    <string name="title_activity_main">PediGochos Servicios</string>
    <string name="package_name">com.pedigochos.service</string>
    <string name="custom_url_scheme">com.pedigochos.service</string>
</resources>
`;
...
if (fs.existsSync(buildGradlePath)) {
  ...
  gradleContent = gradleContent.replace(/applicationId\s+["'][^"']+["']/, 'applicationId "com.pedigochos.service"');
  fs.writeFileSync(buildGradlePath, gradleContent, 'utf8');
  console.log('📱 ApplicationId configurado como independiente: com.pedigochos.service');
}
...
console.log('📦 [5/5] Exportando PediGochos-Servicios.apk...');
const apkSrc = path.join(rootDir, 'android', 'app', 'build', 'outputs', 'apk', 'debug', 'app-debug.apk');
const apkDestProject = path.join(rootDir, 'PediGochos-Servicios.apk');
const userDesktop = path.join(process.env.USERPROFILE || 'C:\\Users\\DerEine', 'Desktop');
const apkDestDesktop = path.join(userDesktop, 'PediGochos-Servicios.apk');
